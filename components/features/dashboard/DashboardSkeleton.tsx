    ...
    <Page>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[...Array(4)].map((_, i) => (
          <div
            key={i}
            className="h-28 bg-slate-800/50 rounded-2xl animate-pulse border border-white/5"
          />
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <Container variant="glass" padding="lg" className="lg:col-span-2">
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex gap-2">
              {[...Array(3)].map((_, i) => (
                <div key={i} className="h-7 w-10 bg-slate-800 rounded-lg animate-pulse" />
              ))}
            </div>
          </div>
          <div className="h-72 w-full bg-slate-800/30 rounded-xl animate-pulse" />
        </Container>
        ...
        <Container variant="glass" padding="none" className="flex flex-col overflow-hidden">
          ...
          <div className="p-4 border-t border-white/5">
            <div className="h-4 w-24 bg-slate-800 rounded animate-pulse mx-auto" />
          </div>
        </Container>
      </div>
    </Page>
