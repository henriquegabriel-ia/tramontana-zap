...
export const UpgradeRoadmapModal: React.FC<UpgradeRoadmapModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-slate-900 border border-[var(--ds-border-subtle)] rounded-2xl w-full max-w-lg mx-4 shadow-2xl animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-hidden flex flex-col">
        {/* Header */}
        <div className="flex items-center gap-4 p-6 border-b border-[var(--ds-border-subtle)] bg-linear-to-r from-primary-500/10 to-transparent shrink-0">
          <div className="p-3 bg-primary-500/20 rounded-xl">
            <TrendingUp className="text-primary-400" size={24} />
          </div>
          <div>
            <h2 className="text-xl font-bold text-white">Aumentar seu Limite</h2>
            <p className="text-sm text-slate-400">Siga o roadmap para evoluir seu tier</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6 overflow-y-auto flex-1">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-slate-800/50 rounded-xl p-4 text-center border border-[var(--ds-border-subtle)]">
              <p className="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Tier Atual</p>
              <p className="text-lg font-bold text-white">{TIER_DISPLAY_NAMES[currentTier]}</p>
              <p className="text-sm text-slate-400">{currentLimit.toLocaleString('pt-BR')}/dia</p>
            </div>
            {nextTier && nextLimit && (
          ...
          {upgradeSteps.length > 0 ? (
            <div className="space-y-3">
              <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Passos para Evoluir</p>
                ...
                return (
                  <div
                    ...
                    className={`p-4 rounded-xl border transition-all \${isCompleted
                      ...
                      : 'bg-slate-800/30 border-[var(--ds-border-subtle)]'
                      }`}
                  >
        ...
        {/* Footer */}
        <div className="flex items-center justify-between p-6 border-t border-[var(--ds-border-subtle)] bg-slate-800/30 shrink-0">
          <a
...
const QualityScoreCard: React.FC<{ qualityScore: string }> = ({ qualityScore }) => {
  const getColorClasses = () => {
    switch (qualityScore) {
      ...
      default:
        return 'bg-slate-800/30 border-[var(--ds-border-subtle)]';
    }
  };
