...
function buildSupportPacketText(params: {
	...
	const lines: string[] = []
	lines.push(`Tramontana Zap · Support Packet · ${new Date().toLocaleString('pt-BR')}`)
...
function buildMissingPermissionsSteps(params: { objectLabel: string; objectId: string }) {
	return [
		...
		'No Tramontana Zap: cole esse token e os IDs corretos em Ajustes → Credenciais WhatsApp.',
...
function buildMissingScopesSteps(missing: string[]) {
	...
	return [
		...
		'No Tramontana Zap: atualize o token em Ajustes → Credenciais WhatsApp e rode o diagnóstico novamente.',
	]
}
	...
	const nextSteps = isMissingPermissionsOrWrongId
		...
		: isTokenInvalid
			? [
				...
				'Atualize as credenciais no Tramontana Zap e tente novamente.',
			]
			: [
...
function buildReportText(
	...
	lines.push(`Tramontana Zap · Diagnóstico Meta/WhatsApp · ${new Date().toLocaleString('pt-BR')}`)
